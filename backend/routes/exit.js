module.exports = (app) => {
  ...
  app.post('/exit', eAdmin,(req, res) => {
    ...
    const sql = 'INSERT INTO exits (product, price, brand, observation, amount, inserted_by) VALUES (?, ?, ?, ?, ?, ?)';
  ...
  app.put('/exit/:id', eAdmin,(req, res) => {
    const id = req.params.id;
    const { product, price, brand, observation, amount, inserted_by } = req.body;
    const sql = `UPDATE exits SET product = ?, price = ?, brand = ?, observation = ?, amount = ?, inserted_by = ? WHERE id = ?`;
